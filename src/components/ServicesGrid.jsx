...
const ServicesGrid = () => {
    return (
        <section className="relative min-h-screen bg-gradient-to-br from-white via-blue-50 to-white text-gray-900 py-13 mt-10 overflow-hidden"> {/* Reduced padding */}
            ...
            {/* Heading */}
            <div className="relative z-10 text-center text-blue-900 pt-10 mb-12"> {/* Reduced top padding and margin below heading */}
            ...
            {/* Services Grid */}
            <div className="relative z-10 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 px-8 sm:px-12 lg:px-24">
                {servicesData.map((service, index) => (
                    <div 
                        key={index} 
                        className={`group relative p-6 rounded-lg shadow-lg transition-all duration-500 transform hover:scale-105 hover:shadow-2xl ${index % 2 === 0 ? 'bg-gradient-to-r from-blue-500 to-blue-700' : 'bg-gradient-to-br from-red-500 to-red-700'}`}
                    >
                        <div className="flex items-center">
                            <img src={service.img} alt={service.title} className="w-16 h-16 rounded-full border-4 border-white shadow-lg mr-4" />
                            <div>
                        ...
                        <div className="absolute inset-0 bg-gray-900 opacity-0 group-hover:opacity-90 transition-opacity duration-500 flex items-center justify-center rounded-lg">
                            <service.icon className="text-6xl text-red-300" />
                        </div>
                    </div>
                ))}
            </div>
            ...
            <div className="relative z-10 text-center mt-16"> {/* Reduced margin above button */}
